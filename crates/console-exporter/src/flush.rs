...
mod tests {
    ...
    fn immediate_should_flush_every_write() {
        ...
        assert!(strategy.should_flush(0), "Immediate: should flush on first write");
        assert!(strategy.should_flush(1), "Immediate: should flush on every write");
        assert!(strategy.should_flush(100), "Immediate: should flush on 100th write");
    }

    #[test]
    fn immediate_on_shutdown_returns_false() {
        let strategy = ImmediateFlush;
        assert!(!strategy.on_shutdown(), "Immediate does not flush on shutdown");
    }
    ...
    fn on_shutdown_should_not_flush_during_writes() {
        ...
        assert!(!strategy.should_flush(5000), "OnShutdown: no flush even after many writes");
    }
    ...
    fn batch_flushes_at_threshold() {
        ...
        assert!(strategy.should_flush(0), "write 0: 0 is multiple of 5, flushes");
        ...
        assert!(strategy.should_flush(5), "write 5: should flush (multiple of 5)");
        assert!(!strategy.should_flush(6), "write 6: should not flush");
        assert!(strategy.should_flush(10), "write 10: should flush (multiple of 5)");
    ...
    fn batch_threshold_of_one_flushes_every_write() {
        ...
        assert!(strategy.should_flush(0), "threshold 1: every write flushes, including 0");
