...
- `Transport::send()` and `TransportError::Backpressure` are repointed to reference `telemetry_types`'s versions of `PayloadEnvelope`/`BackpressureSignal` — this is a reference change, not a data-preserving conversion, because there is currently no data flowing through this unreferenced, dead-code path to preserve.
