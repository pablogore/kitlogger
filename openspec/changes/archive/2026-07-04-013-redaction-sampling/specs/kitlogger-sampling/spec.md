...
Traceability: proposal `openspec/changes/013-redaction-sampling/proposal.md`.
...
- AND the exact sample size and acceptable deviation are a test-implementation concern (see `design.md`/`tasks.md`), not part of this requirement

### Requirement: FR-004 RateLimit Strategy

When `SamplingConfig.strategy` is `RateLimit`, decisions MUST be `true` up to `SamplingConfig.max_events_per_second` times within any rolling one-second window, and `false` for any additional decision requested within that same window.
...
The passage of time used by the `RateLimit` strategy's sliding window MUST be sourced through an injectable clock abstraction, not read directly from the system clock. This MUST allow deterministic testing of window behavior without real time delays.
