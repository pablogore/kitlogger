...
- **Representation** — the specific field layout. This is where the divergence is real: `telemetry_types`'s versions are simpler than the local ones (no `resource`, placeholder `TraceData`/`MetricData`/`LogData` instead of the richer `context_propagation` domain types, no `TelemetryBatchError`, a `u64` instead of a `Duration`, etc.).
...
**Status: unblocked.** The scope below (unchanged from the original) may now proceed.
